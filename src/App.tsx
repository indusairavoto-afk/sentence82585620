  ...
  if (error) {
    return (
      <div className="flex flex-col items-center justify-center w-[180px] h-[320px] rounded-xl border border-zinc-200/50 dark:border-white/10 shadow-sm bg-zinc-100/50 dark:bg-zinc-800/50 backdrop-blur-sm group/img relative overflow-hidden">
        ...
        <span className="text-[10px] px-4 text-center mt-2 text-zinc-400/50">
          Cannot be displayed directly due to authorization.
        </span>
        {isAbsolute && url.startsWith("http") && (
          <a
            ...
            className="text-[10px] text-blue-500 hover:underline opacity-0 group-hover/img:opacity-100 transition-opacity flex items-center gap-1 absolute bottom-3"
          >
    ...
    <div className="flex flex-col items-center gap-1 group/img relative">
      ...
      {isAbsolute && url.startsWith("http") && (
        <a
          ...
          target="_blank"
          rel="noopener noreferrer"
          className="text-[10px] text-blue-500 hover:underline opacity-0 group-hover/img:opacity-100 transition-opacity flex items-center gap-1 mt-1"
        >
...
export default function App() {
  const [stats, setStats] = useState({
    visitors: STAT_BASES.visitors,
    uses: STAT_BASES.uses,
  ...
  const [pendingAction, setPendingAction] = useState<"pdf" | "bridge">(
    "bridge",
  );
    ...
    if (chatData) {
      timer = setTimeout(
        () => {
          ...
          toast.info(
            "Chat session expired for your privacy (7 minutes limit).",
          );
        },
        7 * 60 * 1000,
      ); // 7 minutes
  ...
  const handleExtract = async (
    ...
    if (inputMode === "link") {
      ...
      if (!extensionId && !skipExtensionCheck) {
    ...
    try {
      ...
      if (inputMode === "file") {
          ...
          reader.onprogress = (e) => {
            if (e.lengthComputable) {
              setUploadProgress({
                ...
                percent: Math.round((e.loaded / e.total) * 20),
              });
            }
          };
        ...
        try {
          if (
            ...
            setUploadProgress({
              phase: "Extracting via Extension...",
              percent: 20,
            });
            const extResponse = await Promise.race([
              new Promise<string>((resolve, reject) => {
                (window as any).chrome.runtime.sendMessage(
                  extensionId,
                  { action: "fetch_html", url: shareLink },
                  (response: any) => {
                    if ((window as any).chrome.runtime.lastError) {
                      ...
                      reject(
                        new Error(
                          "Extension failed or timed out. Please try reloading the extension in chrome://extensions or re-download it from our site.",
                        ),
                      );
                    } else {
              ...
              new Promise<string>((_, reject) =>
                setTimeout(
                  () =>
                    reject(
                      new Error(
                        "Extension took too long to respond. The tab might be suspended, or you may need to re-download & reload the latest extension version.",
                      ),
                    ),
                  150000,
                ),
            ...
            if (!response.ok) {
              throw new Error(
                data.message || data.error || "Failed to extract chat",
              );
            }
          ...
          setError({
            ...
            suggestion:
              "Try again or use the Physical Upload method (AI CHAT TO PDF).",
          });
      ...
      try {
        const res = await new Promise<any>((resolve, reject) => {
          ...
          xhr.upload.onprogress = (e) => {
            if (e.lengthComputable && inputMode === "file") {
              ...
              setUploadProgress({
                phase: `Transmitting Data (${(e.loaded / 1024 / 1024).toFixed(1)}MB)...`,
            ...
            progressInterval = setInterval(() => {
              ...
              if (simulatedProgress < 90) {
                const phases = [
                  ...
                  "Extracting Chat Nodes...",
                  "Formatting Messages...",
                ];
                const currentPhase =
                  phases[Math.floor((simulatedProgress - 70) / 7)] || phases[2];
                setUploadProgress({
                  phase: currentPhase,
          ...
          xhr.onload = () => {
            ...
            try {
              ...
              if (xhr.status === 404)
                errorMessage =
                  "Endpoint not found. Make sure your server is deployed with the latest code.";
              else if (xhr.status === 413)
                errorMessage =
                  "Payload too large. File size exceeds server limits.";
              else if (xhr.status === 403) errorMessage = "Request blocked.";
              else if (xhr.status >= 500) errorMessage = "Server error.";
              reject(
                new Error(
                  `Server error (Status ${xhr.status}). ${errorMessage}`,
                ),
              );
            }
          };
        ...
        if (!res.ok) {
          ...
          toast.error(
            `Extraction failed: ${res.data.message || res.data.error || "Unknown error"}`,
          );
          setUploadProgress(null);
          setLoading(false);
          return;
        ...
        toast.success("Successfully extracted chat!");
        setUploadProgress({
          phase: "Extraction Complete! Bridging...",
          percent: 100,
        });
  ...
  return (
    <div
      className={`min-h-screen flex flex-col overflow-x-hidden transition-colors duration-300 ${theme === "dark" ? "dark bg-[#0a0a0a] text-zinc-100 selection:bg-zinc-800 selection:text-white relative" : "bg-white text-zinc-900 selection:bg-zinc-200 selection:text-black relative"}`}
    >
      ...
      <div className="absolute inset-0 z-0 pointer-events-none opacity-50 dark:opacity-[0.15]">
        ...
        >
          <div
            className="relative left-[calc(50%-11rem)] aspect-[1155/678] w-[36.125rem] -translate-x-1/2 rotate-[30deg] bg-gradient-to-tr from-[#8086ff] to-[#80ffc7] opacity-20 sm:left-[calc(50%-30rem)] sm:w-[72.1875rem]"
            style={{
              clipPath:
        ...
        >
          <div className="w-11 h-11 bg-black dark:bg-white rounded-xl flex items-center justify-center shadow-sm shrink-0 overflow-hidden">
            <img
              ...
              alt="Seamless Bridge Logo"
              className="w-8 h-8 object-contain transition-all invert dark:invert-0"
            />
          </div>
          <span className="text-xl font-bold tracking-[0.10em] uppercase font-mono bg-clip-text text-transparent bg-gradient-to-r from-zinc-900 to-zinc-500 dark:from-white dark:to-zinc-500 leading-none">
            SEAMLESS
            <br />
            BRIDGE
          </span>
        ...
        <nav className="flex items-center gap-2">
          <div className="hidden lg:flex items-center gap-6 font-medium text-xs uppercase tracking-widest text-zinc-500 mr-4">
            <button
              ...
              className={`transition-colors ${currentTab === "about" ? "text-zinc-900 dark:text-zinc-100 font-bold" : "hover:text-zinc-900 dark:hover:text-zinc-300"}`}
            >
              About
            </button>
            <button
              onClick={() => setCurrentTab("impact")}
              className={`transition-colors ${currentTab === "impact" ? "text-zinc-900 dark:text-zinc-100 font-bold" : "hover:text-zinc-900 dark:hover:text-zinc-300"}`}
            >
              Impact
            </button>
            <button
              onClick={() => setCurrentTab("faq")}
              className={`transition-colors ${currentTab === "faq" ? "text-zinc-900 dark:text-zinc-100 font-bold" : "hover:text-zinc-900 dark:hover:text-zinc-300"}`}
            >
              FAQ
            </button>
          </div>
          ...
          <button
            ...
            className="hidden md:flex items-center gap-2 bg-zinc-900 dark:bg-white text-white dark:text-zinc-900 px-4 py-2 rounded-full font-bold text-xs uppercase tracking-widest hover:bg-zinc-800 dark:hover:bg-zinc-200 transition-colors mr-2"
          >
          ...
          <button
            onClick={toggleTheme}
            className="w-10 h-10 rounded-full border border-zinc-200 dark:border-white/10 text-zinc-500 hover:text-zinc-900 dark:hover:text-white hover:bg-zinc-100 dark:hover:bg-white/5 transition-all flex items-center justify-center shrink-0"
          >
          ...
          {user && (
            <button
              onClick={logOut}
              title="Sign Out"
              className="w-10 h-10 rounded-full border border-zinc-200 dark:border-white/10 text-zinc-500 hover:text-red-500 dark:hover:text-red-400 hover:bg-zinc-100 dark:hover:bg-white/5 transition-all flex items-center justify-center shrink-0 ml-1"
            >
              <LogOut size={14} />
            </button>
          )}
        ...
        {currentTab === "vault" ? (
          ...
          <>
            <div className="w-full max-w-2xl text-center mb-0 relative">
              <h1 className="text-4xl md:text-6xl font-extrabold tracking-tighter mb-6 bg-clip-text text-transparent bg-gradient-to-b from-zinc-900 to-zinc-500 dark:from-white dark:to-zinc-500">
                Seamless AI Continuity
              </h1>
              <p className="text-zinc-600 dark:text-zinc-400 text-sm md:text-base tracking-wide max-w-lg mx-auto leading-relaxed mb-2">
                Upload an exported HTML chat or paste a Share Link to bridge the
                gap between different LLMs flawlessly.
              </p>
            </div>
              ...
              {loading ? (
                ...
                >
                  <div className="flex flex-col md:flex-row items-start md:items-center justify-between mb-4 gap-4">
                    <h3 className="text-[10px] uppercase tracking-[0.3em] text-zinc-500 font-bold bg-zinc-100 dark:bg-zinc-900 px-3 py-1 rounded inline-flex">
                      Bridged Conversion
                    </h3>
                  </div>
                  ...
                  <div className="bg-white/50 backdrop-blur-sm dark:bg-[#0a0a0a]/50 border border-zinc-200 dark:border-white/10 overflow-hidden mb-12 rounded-xl shadow-2xl p-12 lg:p-24 flex flex-col items-center justify-center min-h-[400px]">
                    ...
                    <h2 className="text-sm md:text-base font-bold font-mono tracking-[0.2em] uppercase text-zinc-900 dark:text-zinc-100 mb-2 animate-[pulse_2s_ease-in-out_infinite]">
                      Communication Ping
                    </h2>
                ...
                >
                  <div className="w-full max-w-2xl border border-zinc-200/50 dark:border-transparent p-[1.5px] bg-zinc-200/50 dark:bg-zinc-800/50 mb-12 rounded-2xl shadow-2xl overflow-hidden relative isolate transform-gpu">
                      ...
                      <form onSubmit={handleExtract} className="w-full">
                        {inputMode === "file" ? (
                          ...
                          >
                            ...
                            {htmlFile ? (
                              ...
                              >
                                ...
                                <span className="text-zinc-900 dark:text-white font-bold text-xl truncate px-8 max-w-[80%] text-center">
                                  {htmlFile.name}
                                </span>
                                <span className="text-xs text-zinc-500 mt-3 font-mono uppercase tracking-widest">
                                  {(htmlFile.size / 1024).toFixed(1)} KB • Ready
                                  to extract
                                </span>
                              </motion.div>
                            ) : (
                              ...
                              >
                                <div
                                  className={`w-16 h-16 rounded-full flex items-center justify-center mb-5 transition-colors duration-300 shadow-sm ${
                                    isDragging
                                      ? "bg-black text-white dark:bg-white dark:text-black shadow-xl ring-4 ring-black/5 dark:ring-white/10"
                                      : "bg-white dark:bg-zinc-900 text-zinc-400 dark:text-zinc-500 shadow-md border border-zinc-200 dark:border-zinc-800/50"
                                  }`}
                                >
                                  <Download
                                    size={24}
                                ...
                                <h3 className="text-zinc-900 dark:text-white font-bold text-xl mb-2 tracking-tight">
                                  Upload HTML Export
                                </h3>
                                <p className="text-sm text-zinc-500 dark:text-zinc-400 max-w-[260px] text-center leading-relaxed">
                                  Drop your saved chat page here, or click to
                                  browse files.
                                </p>
                            ...
                            <div
                              className={`absolute inset-0 bg-gradient-to-br from-black/[0.02] to-transparent dark:from-white/[0.03] dark:to-transparent pointer-events-none transition-opacity duration-500 ${isDragging ? "opacity-100" : "opacity-0"}`}
                            />
                          </motion.div>
                        ) : (
                          <div className="w-full h-[280px] rounded-xl border-2 border-zinc-200 dark:border-white/5 bg-zinc-50/50 dark:bg-black/40 flex flex-col items-center justify-center p-8 mb-8 relative shadow-inner">
                            ...
                            <h3 className="text-zinc-900 dark:text-white font-bold text-xl mb-6 tracking-tight z-10">
                              Paste Share Link
                            </h3>
                            ...
                            <div className="relative w-full max-w-md z-10 mt-3 flex justify-center">
                              {extensionId ? (
                                ...
                                <button
                                  ...
                                  className="flex items-center gap-1.5 px-3 py-1.5 bg-zinc-100 dark:bg-zinc-800 text-zinc-500 hover:text-zinc-900 dark:hover:text-white rounded-full text-[10px] uppercase tracking-widest font-bold transition-all"
                                >
                            ...
                            <div className="p-5 flex flex-col gap-4">
                              ...
                              {error.suggestion && (
                                <div className="bg-zinc-50 dark:bg-black/40 p-4 border-l-2 border-red-500/50">
                                  <h4 className="text-[9px] uppercase tracking-widest text-zinc-500 dark:text-zinc-500 mb-2 font-bold">
                                    Recommended Action
                                  </h4>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 border-t border-zinc-200 dark:border-zinc-900">
                      <div className="p-6 border-b md:border-b-0 md:border-r border-zinc-200 dark:border-zinc-900">
                        <label className="text-[10px] uppercase tracking-widest text-zinc-500 dark:text-zinc-500 dark:text-zinc-600 block mb-3">
                          Source Data
                        </label>
                  ...
                  <div className="w-full max-w-2xl mt-4 mb-8">
                    <div className="text-center mb-8">
                      <h3 className="text-sm uppercase tracking-[0.2em] text-zinc-500 font-bold mb-2">
                        How it works
                      </h3>
                      <p className="text-zinc-600 dark:text-zinc-400 text-xs mb-8">
                        Extract your conversations in 3 simple steps
                      </p>
                      ...
                      <div className="w-full flex justify-center mb-8 bg-zinc-50/50 dark:bg-black/20 rounded-2xl border border-zinc-100 dark:border-zinc-800/50 p-4 sm:p-8 shadow-sm">
                        ...
                        >
                          ...
                          />
                          <path
                            d="M40 54 V100 M40 70 Q 60 60 90 40 M40 70 L 20 90 M40 100 L 25 130 M40 100 L 55 130"
                            stroke="currentColor"
                            strokeWidth="3"
                            strokeLinecap="round"
                          ...
                          <g transform="translate(140, 45)">
                            ...
                            />
                            <path
                              d="M12 15 h26 M12 25 h20 M12 35 h14"
                              stroke="currentColor"
                              strokeWidth="2"
                              strokeLinecap="round"
                            />
                            <path
                              d="M35 30 l8 8 l-2 -8 z"
                              fill="currentColor"
                            />
                          </g>
                          ...
                          {inputMode === "file" ? (
                            <>
                              ...
                              <g transform="translate(390, 40)">
                                <path
                                  d="M0 0 L35 0 L50 15 L50 60 L0 60 Z"
                                  fill="currentColor"
                                  fillOpacity="0.05"
                                  stroke="currentColor"
                                  strokeWidth="2"
                                ...
                                />
                                <path
                                  d="M35 0 L35 15 L50 15"
                                  stroke="currentColor"
                                  strokeWidth="2"
                                  strokeLinejoin="round"
                                />
                                <path
                                  d="M10 30 L16 25 L10 20 M20 30 L26 25 L20 20 M30 30 L35 30"
                                  stroke="currentColor"
                                  strokeWidth="2"
                                  strokeLinecap="round"
                                  strokeLinejoin="round"
                            ...
                            <>
                              ...
                              <g transform="translate(260, 45)">
                                ...
                                {/* Link Icon */}
                                <path
                                  d="M 25 29 h -2 a 4 4 0 0 1 0 -8 h 2 M 35 21 h 2 a 4 4 0 0 1 0 8 h -2 M 25 25 h 10"
                                  stroke="currentColor"
                                  strokeWidth="2"
                                  strokeLinecap="round"
                                  strokeLinejoin="round"
                      ...
                      {inputMode === "file" ? (
                        <>
                          <div className="flex flex-col items-center text-center p-6 bg-zinc-100/50 dark:bg-zinc-900/50 border border-zinc-200 dark:border-zinc-800 rounded-xl relative">
                            <div className="w-8 h-8 rounded-full bg-black text-white dark:bg-white dark:text-black flex items-center justify-center font-bold text-xs mb-4 absolute -top-4 shadow-lg uppercase tracking-widest font-mono">
                              1
                            </div>
                            ...
                            <p className="text-xs text-zinc-500 dark:text-zinc-400 leading-relaxed">
                              Navigate to your finished conversation in ChatGPT,
                              Claude, or any AI platform.
                            </p>
                          </div>
                          ...
                          <div className="flex flex-col items-center text-center p-6 bg-zinc-100/50 dark:bg-zinc-900/50 border border-zinc-200 dark:border-zinc-800 rounded-xl relative">
                            ...
                            <p className="text-xs text-zinc-500 dark:text-zinc-400 leading-relaxed">
                              ...
                              ) on your keyboard and save as "Webpage, HTML
                              Only".
                            </p>
                          </div>
                          ...
                          <div className="flex flex-col items-center text-center p-6 bg-zinc-100/50 dark:bg-zinc-900/50 border border-zinc-200 dark:border-zinc-800 rounded-xl relative">
                            ...
                            <p className="text-xs text-zinc-500 dark:text-zinc-400 leading-relaxed">
                              Drag & drop the saved HTML file into the dropzone
                              above to extract the chat.
                            </p>
                          </div>
                        </>
                      ) : (
                        <>
                          ...
                          <div className="flex flex-col items-center text-center p-6 bg-zinc-100/50 dark:bg-zinc-900/50 border border-zinc-200 dark:border-zinc-800 rounded-xl relative">
                            ...
                            <p className="text-xs text-zinc-500 dark:text-zinc-400 leading-relaxed">
                              Click the Share icon in the top right corner and
                          ...
                          <div className="flex flex-col items-center text-center p-6 bg-zinc-100/50 dark:bg-zinc-900/50 border border-zinc-200 dark:border-zinc-800 rounded-xl relative">
                            ...
                            <p className="text-xs text-zinc-500 dark:text-zinc-400 leading-relaxed">
                              Paste the copied share link into the input field
                              above to parse the conversation.
                            </p>
                          </div>
                        </>
                      )}
                ...
                >
                  ...
                  <div className="bg-white/50 backdrop-blur-xl dark:bg-[#0a0a0a]/50 border border-zinc-200/50 dark:border-white/10 overflow-hidden mb-12 rounded-2xl shadow-2xl relative">
                    ...
                    <div className="flex flex-col md:flex-row md:items-center justify-between p-5 border-b border-zinc-200/50 dark:border-white/5 bg-zinc-50/50 dark:bg-black/20 gap-4 relative z-10">
                      <div className="font-sans text-[13px] text-zinc-900 dark:text-white font-bold truncate leading-tight select-all">
                        {chatData.title}
                      </div>
                      <div className="flex items-center gap-4 flex-wrap">
                        <span className="text-[9px] uppercase tracking-[0.2em] text-zinc-500 font-mono flex items-center gap-1">
                          <FileText size={10} /> {chatData.messages.length} msgs
                        </span>
                        <div className="flex gap-2 flex-wrap">
                          <button
                            onClick={async () => {
                              try {
                                setCopied("loading");
                                setLinkStatus({
                                  step: "Formatting conversation payload...",
                                  progress: 10,
                                });
                                await new Promise((r) => setTimeout(r, 400));

                                setLinkStatus({
                                  step: "Transmitting to secure pastebin...",
                                  progress: 45,
                                });
                                const res = await fetch("/api/public-bridge", {
                                  ...
                                  body: JSON.stringify({
                                    text: formatAsPrompt(chatData),
                                  }),
                                });

                                setLinkStatus({
                                  step: "Generating text-only public endpoint...",
                                  progress: 85,
                                });
                                const { url } = await res.json();

                                setLinkStatus({
                                  step: "Finalizing bridge...",
                                  progress: 100,
                                });
                          ...
                          >
                            ...
                            {copied === "bridge" ? (
                              ...
                              <Loader2
                                size={12}
                                className="animate-spin relative z-10 text-white/70 dark:text-black/70"
                              />
                            ) : (
                              <LinkIcon
                                size={12}
                                className="relative z-10 text-white/70 dark:text-black/70"
                              />
                            )}
                            <span className="relative z-10">
                              {copied === "bridge"
                          ...
                          >
                            <Download size={12} className="text-white" /> Edit
                            PDF
                          </button>
                          <button
                            onClick={() =>
                    ...
                    <div className="bg-zinc-50/50 dark:bg-[#0a0a0a]/50 border-b border-zinc-200/50 dark:border-white/5 p-3 flex flex-col md:flex-row md:items-center justify-between gap-4 backdrop-blur-sm relative z-10">
                      <div className="text-[10px] text-zinc-600 dark:text-zinc-400 font-mono flex items-center gap-3">
                        <span className="text-zinc-900 dark:text-white bg-white dark:bg-black border border-zinc-200/50 dark:border-white/10 px-2 py-0.5 rounded shadow-sm uppercase tracking-widest text-[8px] font-bold">
                          Tip
                        </span>
                        Claude and Grok do not support secure automatic link
                        extraction. Use HTML export instead.
                      </div>
                    ...
                    <div className="divide-y divide-zinc-200/50 dark:divide-white/5 bg-white dark:bg-[#0a0a0a] max-h-[600px] overflow-y-auto">
                      ...
                      {filteredMessages.map((msg, idx) => (
                        ...
                        >
                          <button
                            onClick={() =>
                              copyToClipboard(msg.content, `msg-${idx}`)
                            }
                          ...
                          <div
                            className={`flex flex-col w-full max-w-[85%] md:max-w-3xl ${msg.role === "user" ? "items-end" : "items-start"}`}
                          >
                            {msg.role !== "unknown" && (
                            ...
                            <div
                              className={`flex-1 min-w-0 prose dark:prose-invert prose-p:leading-relaxed prose-pre:bg-zinc-100 dark:prose-pre:bg-zinc-900/80 prose-pre:border prose-pre:border-zinc-200/50 dark:prose-pre:border-white/5 prose-pre:rounded-xl prose-pre:shadow-inner max-w-none text-sm font-sans relative z-10 w-full ${
                                msg.role === "user"
                            ...
                            >
                              {msg.images && msg.images.length > 0 && (
                                <div
                                  className={`flex flex-col gap-2 ${msg.role === "user" ? "items-end mb-4" : "items-start mb-4"}`}
                                >
                              ...
                              {msg.content.includes("Uploaded image") &&
                                ...
                                (!msg.images || msg.images.length === 0) && (
                                  <div className="mt-3 text-xs opacity-80 bg-black/5 dark:bg-white/5 p-3 rounded-lg border border-black/10 dark:border-white/10 flex flex-col gap-1.5">
                                    ...
                                    <span>
                                      OpenAI currently removes images from
                                      shared links for privacy reasons. Because
                                      of this, we can't extract it from the
                                      link. To include images, please save the
                                      ChatGPT page as an HTML file (Cmd/Ctrl +
                                      S) and use the "Local HTML File" upload
                                      option instead.
                                    </span>
                                  </div>
                                )}
      ...
      <footer className="px-4 sm:px-6 md:px-12 py-6 sm:py-8 border-t border-zinc-200/50 dark:border-white/5 flex flex-col md:flex-row gap-4 justify-between items-center bg-white/50 dark:bg-[#0a0a0a]/50 backdrop-blur-xl mt-auto shrink-0 z-10 relative">
        <div className="text-[10px] uppercase tracking-[0.2em] text-zinc-500 font-mono flex items-center gap-2">
          ...
          <span className="opacity-50">•</span> V2.0-Alpha
        </div>
        ...
        {showExtensionModal && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-zinc-900/40 dark:bg-black/80 backdrop-blur-md">
            <motion.div
              ...
              className="bg-white dark:bg-[#0a0a0a] rounded-2xl border border-zinc-200 dark:border-white/10 p-8 max-w-md w-full shadow-2xl relative"
            >
              <button
              ...
              <h2 className="text-2xl font-bold text-zinc-900 dark:text-white tracking-tight mb-2">
                Wait! Extension Recommended
              </h2>
              <p className="text-zinc-600 dark:text-zinc-400 text-sm leading-relaxed mb-6">
                Because of recent updates to anti-bot systems, Cloud extraction
                often fails for links. We strongly recommend installing our free
                open-source Chrome extension to extract the page locally from
                your browser.
              </p>
              <div className="bg-zinc-50 dark:bg-black/50 p-4 border border-zinc-200 dark:border-white/5 rounded-xl mb-6">
                ...
                <ol className="text-sm text-zinc-700 dark:text-zinc-300 space-y-2 list-decimal pl-4">
                  <li>
                    ...
                    to view the installation page.
                  </li>
                  <li>Follow instructions to download and install.</li>
                  <li>
                    The extension links automatically so you don't even need to
                    refresh!
                  </li>
                </ol>
              </div>
              <div className="flex flex-col gap-3">
                <button
                  ...
                  className="w-full bg-zinc-900 dark:bg-white text-white dark:text-black font-bold text-sm py-3 rounded-xl hover:bg-zinc-800 dark:hover:bg-zinc-100 transition-colors uppercase tracking-widest mt-1"
                >
                ...
                <button
                  onClick={() => {
                    setShowExtensionModal(false);
                    handleExtract(undefined, pendingAction, true, true);
                  }}
                  className="w-full text-[10px] text-zinc-500 hover:text-zinc-700 dark:hover:text-zinc-300 uppercase tracking-[0.1em] mt-3 underline underline-offset-4 opacity-70 hover:opacity-100 transition-all font-bold"
                >
                  Try Cloud Extraction Anyway
                </button>
              </div>
            </motion.div>
          </div>
